...
if __name__ == "__main__":
    ...
    with open(inputfile,"r") as f:
        for line in f.readlines():
            ...
            else: inputseqs[-1]+=line.strip()
    print(len(inputseqs))

    tempfile="example.fasta"


    for inputidx in range(len(inputnames)):
        with open(tempfile, "w") as f:
            f.write(inputnames[inputidx])
            f.write(inputseqs[inputidx])
        ...
        for temp_value in temperature_range:
            scr, o1 = CallRNAfold(tempfile, temp_value)
            # CREATES:
            ...
            definition = open(tempfile, "r").readline()[1:-1]
